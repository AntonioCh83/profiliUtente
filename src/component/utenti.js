...
import { APRI_MODAL, CHIUDI_MODAL } from "../utils/action";
  ...
  return (
    <section className="card d-flex shadow align-item-center p-4 mt-3 border-primary">
      ...
      { dati.map((el,index)=>{
        ...
        return (
        <article key={id} className="card shadow-lg mt-4 border-primary" style={{backgroundColor:"#a2a2a2"}}>
          ...
          <div className="row justify-content-center">
            <button  onClick={()=>apriModal(index,dati)} 
              className="button button-info mb-2 col-3">
              Info e-mail {id}
            </button>
          </div>
        </article>);
